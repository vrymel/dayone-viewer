...
export function VideoDisplay({ video }: VideoDisplayProps) {
    ...
    const loadVideo = async (videoLocation: string, videoFilename: string): Promise<void> => {
        ...
        try {
            const result = await window.api.getVideo(videoLocation, videoFilename.trim())

            if (result) {
                setVideoUrl(result)
            } else {
                setError('Failed to load video. Check if the file exists and is a valid video.')
                setVideoUrl(null)
            }
        } catch (err) {
